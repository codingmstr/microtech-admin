...
export default function Loader ({ className='' }) {
    ...
    return (
        ...
        <div className={`screen_loader rounded-lg animate__animated w-full ${className.includes('fixed') ? 'fixed' : 'absolute'} ${className.includes('bg') ? 'bg-panel dark:bg-panel-dark' : 'bg-body dark:bg-body-dark'} ${config.user?.logged ? className.includes('container') ? 'h-[calc(100vh_-_160px)]' : 'h-full' : 'h-screen'} ${className} inset-0 z-[60] grid place-content-center`}>
